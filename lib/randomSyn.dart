...
class Randomsyn extends StatefulWidget {
  @override
  _RandomsynState createState() => _RandomsynState();
}

class _RandomsynState extends State<Randomsyn> {
  SqliteHelper helper;
  ...
  randomWord() async {
    ...
    if (count == 0) {
      const sql = 'SELECT ethai FROM eng2th WHERE id=?';
      ...
      if (list.length > 0) {
        ...
        _currentEn = test.substring(9, test.length - 2);
        if (_currentEn == "null") {
          _currentEn = "NO SYNONYM !";
        }
      ...
      const sql = 'SELECT esyn FROM eng2th WHERE id=?';
      List list = await _db.rawQuery(sql, [randomNumber]);

      const sql2 = 'SELECT eentry FROM eng2th WHERE id=?';
      List list2 = await _db.rawQuery(sql2, [randomNumber]);
      ...
      if (list.length > 0) {
        ...
        // _currentEn = list.toString();
        String test = list.toString();
        _currentEn = test.substring(8, test.length - 2);
        if (_currentEn == "null") {
          _currentEn = "NO SYNONYM !";
        }
